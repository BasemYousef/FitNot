...
public class EnemyHandler : MonoBehaviour {
    ...
    private const float TARGET_DISTANCE = 200f;
    ...
    public static EnemyHandler Create(Vector3 position) {
        ...
        enemyHandler.SetGetTarget(() => CaptainAmerica.instance);
        return enemyHandler;
    }
    ...
    private void Update() {
        ...
        case State.Attacking:
            float stopAttackingRange = 25f;
            if (getEnemyTarget == null || Vector3.Distance(getEnemyTarget().GetPosition(), GetPosition()) > stopAttackingRange) {
                // Stop attacking
                state = State.Normal;
            }
            break;
    ...
    private void FindTarget() {
        float targetRange = TARGET_DISTANCE;
    ...
    public void Knockout(Vector3 fromPosition) {
        health = 0;
        Vector3 flyDir = (GetPosition() - fromPosition).normalized;
        FlyingBody.Create(GameAssets.i.pfEnemyFlyingBody, GetPosition(), flyDir);
        Destroy(gameObject);
    }
